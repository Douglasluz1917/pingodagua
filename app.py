...
with aba2:
    ...
    if df_historico.empty:
        ...
        grafico_financeiro = alt.Chart(df_grafico_caixa).mark_area(
            color=alt.Gradient(
                gradient='linear',
            ...
            x=alt.X("Mês/Ano:O", sort=None, title="Mês"),
    ...
    with st.form("form_textos"):
        ...
        if st.form_submit_button("💾 Salvar Textos e PIX", use_container_width=True):
            novas_configs = {
                ...
                "msg_vencendo": nova_msg_vencendo,
                "msg_atrasado": nova_msg_atrasado,
                "msg_aniversario": nova_msg_niver
            }
            with open(arquivo_textos, "w", encoding="utf-8") as f:
                json.dump(novas_configs, f, ensure_ascii=False, indent=4)
